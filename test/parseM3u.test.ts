...
describe('parseM3u', () => {
  it('parses a well-formed catalog with attributes', () => {
    ...
    expect(entries).toEqual([
      ...
      { id: 'bs11.jp', name: 'BS11', url: 'https://example.com/bs11/index.m3u8' },
    ]);
  });

  it('omits logo/chno when the attributes are absent or empty', () => {
    const { entries } = parseM3u(
      ['#EXTINF:-1 tvg-id="a" tvg-logo="" tvg-chno="",Chan A', 'http://a/'].join('\n'),
    );
    expect(entries).toEqual([{ id: 'a', name: 'Chan A', url: 'http://a/' }]);
    expect(entries[0]).not.toHaveProperty('logo');
    expect(entries[0]).not.toHaveProperty('chno');
  });
  ...
  it('takes the display name after the LAST comma — commas inside attribute values never bleed in', () => {
    const { entries, warnings } = parseM3u(
      ['#EXTINF:-1 tvg-id="n" tvg-logo="http://logo/a,b.png" group-title="News, World",News Channel', 'http://n/'].join(
        '\n',
      ),
    );
    expect(warnings).toEqual([]);
  ...
  it('truncates a display name containing a literal comma to the part after it (documented tradeoff)', () => {
    const { entries } = parseM3u(['#EXTINF:-1 tvg-id="hw",Hello, World', 'http://hw/'].join('\n'));
  ...
  it('caps derived ids at 64 chars without a trailing hyphen', () => {
    ...
    const { entries } = parseM3u([`#EXTINF:-1,${name}`, 'http://long/'].join('\n'));
  ...
  it('skips an entry whose id cannot be derived (non-sluggable name, no tvg-id)', () => {
    const { entries, warnings } = parseM3u(['#EXTINF:-1,日本語チャンネル', 'http://jp/'].join('\n'));
    expect(entries).toEqual([]);
  ...
  it('suffixes duplicate DERIVED ids too (two entries slugging to the same name)', () => {
    const { entries, warnings } = parseM3u(
      ['#EXTINF:-1,My Channel', 'http://1/', '#EXTINF:-1,My  Channel!', 'http://2/'].join('\n'),
    );
  ...
  it('warns and skips a bare URL without a preceding #EXTINF', () => {
    const { entries, warnings } = parseM3u(
      ['#EXTM3U', 'http://bare/', '#EXTINF:-1 tvg-id="ok",OK', 'http://ok/'].join('\n'),
    );
    expect(entries.map((e) => e.id)).toEqual(['ok']);
    expect(warnings).toHaveLength(1);
  ...
  it('ignores blank lines and unknown # directives between EXTINF and URL', () => {
    const { entries, warnings } = parseM3u(
      ['#EXTINF:-1 tvg-id="s",Spaced', '', '#EXTGRP:anime', '  ', 'http://s/'].join('\n'),
    );
    expect(warnings).toEqual([]);
    expect(entries).toEqual([{ id: 's', name: 'Spaced', url: 'http://s/' }]);
  });
